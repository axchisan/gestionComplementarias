...
import { Menu, X, User, FileText, BarChart3, Home, LogOut, Users, Settings, CheckCircle, Bell } from "lucide-react"
...
export function Navigation() {
  ...
  const loadNotificationCount = async () => {
    try {
      const response = await fetch("/api/notificaciones?limit=1", {
        headers: {
      ...
      if (response.ok) {
        ...
        setNotificationCount(data.noLeidas || 0)
      }
    } catch (error) {
